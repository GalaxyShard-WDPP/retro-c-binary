...
var ASM_CONSTS = {
  31644: function() {FS.syncfs(function(err) { if (err) console.log('Error syncing files: ', err); });},  
 31730: function($0, $1) {FS.syncfs(function(err) { if (err) console.log('Error syncing files: ', err); ccall('sync_finished', 'void', ['number', 'number', 'number'], [err ? 0 : 1, $0, $1]); });},  
 31903: function() {FS.mkdir('/gamedata'); FS.mount(IDBFS, {}, '/gamedata');},  
 31964: function() {FS.syncfs(true, function(err) { if (err) console.log('Error syncing file system: ', err); ccall('readyToInit', 'void'); });},  
 32092: function() {var htmlStyles = window.getComputedStyle(document.querySelector("html")); return (parseInt(htmlStyles.getPropertyValue("--glfm-chrome-top-old")) || 0) + (parseInt(htmlStyles.getPropertyValue("--glfm-chrome-top")) || 0);},  
 32316: function() {var htmlStyles = window.getComputedStyle(document.querySelector("html")); return (parseInt(htmlStyles.getPropertyValue("--glfm-chrome-right-old")) || 0) + (parseInt(htmlStyles.getPropertyValue("--glfm-chrome-right")) || 0);},  
 32544: function() {var htmlStyles = window.getComputedStyle(document.querySelector("html")); return (parseInt(htmlStyles.getPropertyValue("--glfm-chrome-bottom-old")) || 0) + (parseInt(htmlStyles.getPropertyValue("--glfm-chrome-bottom")) || 0);},  
 32774: function() {var htmlStyles = window.getComputedStyle(document.querySelector("html")); return (parseInt(htmlStyles.getPropertyValue("--glfm-chrome-left-old")) || 0) + (parseInt(htmlStyles.getPropertyValue("--glfm-chrome-left")) || 0);},  
 33000: function() {return (('ontouchstart' in window) || (navigator.msMaxTouchPoints > 0));},  
 33077: function($0) {var emCursors = new Array('auto', 'none', 'default', 'pointer', 'crosshair', 'text'); Module['canvas'].style.cursor = emCursors[$0];},  
 33214: function() {var canvas = Module['canvas']; var devicePixelRatio = window.devicePixelRatio || 1; canvas.width = canvas.clientWidth * devicePixelRatio; canvas.height = canvas.clientHeight * devicePixelRatio;},  
 33412: function() {var canvas = Module['canvas']; return canvas.width;},  
 33468: function() {var canvas = Module['canvas']; return canvas.height;},  
 33525: function() {var canvas = Module['canvas']; var devicePixelRatio = window.devicePixelRatio || 1; var width = canvas.clientWidth * devicePixelRatio; var height = canvas.clientHeight * devicePixelRatio; if (width != canvas.width || height != canvas.height) { canvas.width = width; canvas.height = height; return 1; } else { return 0; }},  
 33850: function($0, $1, $2, $3) {var rect = Module['canvas'].getBoundingClientRect(); setValue($0, rect.x, "float"); setValue($1, rect.y, "float"); setValue($2, rect.width, "float"); setValue($3, rect.height, "float");}
};
